...
int main(int argc, char *argv[])
{
    ...
    auto& server = Lbs::Server::GetInstance();

    if (Sync::Signal::GetInstance().SetHandler(Sync::Signal::kSIGTERM, Lbs::Server::OnStopSignal)) {
        Trace(0, "can't setup signal handler\n");
        return -1;
    }

    if (Sync::Signal::GetInstance().SetHandler(Sync::Signal::kSIGINT, Lbs::Server::OnStopSignal)) {
        Trace(0, "can't setup signal handler\n");
        return -1;
    }

    if (Sync::Signal::GetInstance().SetHandler(Sync::Signal::kSIGPIPE, Lbs::Server::OnSigPipe)) {
        Trace(0, "can't setup signal handler\n");
        return -1;
    }
